...
public class ModuleOverviewBuilder {
    ...
    public static ModuleOverview create(final ModuleGuide guide) {
        ...
        final List<Module> sortedModules =
            guide.modules()
            .stream()
            .sorted(ModuleOverviewBuilder.createOverviewComparator(guide.specializationOrder()))
            .toList();
        for (final Module module : sortedModules) {
            final RawModule rawModule = module.module();
            final MetaModule meta = module.meta();
            final BigFraction contactHoursFactor = ModuleOverviewBuilder.parseFactor(meta.contacthoursfactor());
            final BigFraction homeHoursFactor = ModuleOverviewBuilder.parseFactor(meta.homehoursfactor());
            final BigFraction ectsFactor = ModuleOverviewBuilder.parseFactor(meta.ectsfactor());
            ...
            if (
                !ModuleOverviewBuilder.isInt(contactHours)
                || !ModuleOverviewBuilder.isInt(homeHours)
                || !ModuleOverviewBuilder.isInt(ects)
            ) {
            ...
            if (meta.specialization() != null) {
                ...
                if (elective) {
                    if (electiveModulesMap.containsKey(meta.specializationnumber())) {
                        ...
                        semesterMap.merge(
                            meta.semester(),
                            List.of(statsForSemester),
                            ModuleOverviewBuilder::concatLists
                        );
                    }
                } else {
                ...
                ectsSum += ects.intValue();
                weightSum += meta.weight();
                semesterMap.merge(meta.semester(), List.of(stats), ModuleOverviewBuilder::concatLists);
            }
        }
    ...
    private static BigFraction parseFactor(final String factor) {
        return factor == null ? BigFraction.ONE : ModuleOverviewBuilder.parseRationalNumber(factor);
    }
